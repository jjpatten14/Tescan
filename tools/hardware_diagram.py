...
Parts: ESP32 DevKit V1, SN65HVD230 module, P6KE6.8CA TVS (x2),
       LM2596 buck, 1N5819 Schottky, 3A ATO fuse.
"""
import matplotlib
matplotlib.use('Agg')
...
def gnd_sym(x, y, c):
    ax.plot([x, x], [y+0.28, y+0.13], color=c, lw=2, zorder=5)
    ax.plot([x-0.14, x+0.14], [y+0.13, y+0.13], color=c, lw=2, zorder=5)
    ax.plot([x-0.09, x+0.09], [y+0.06, y+0.06], color=c, lw=1.5, zorder=5)
    ax.plot([x-0.04, x+0.04], [y,      y     ], color=c, lw=1, zorder=5)
...
# ─── P6KE6.8CA TVS Diodes (bidirectional, DO-15 axial, through-hole) ─────────
# Two shunt diodes: one on CANH, one on CANL — both clamp to GND
T1X = 3.05   # CANH TVS junction x on wire
T2X = 3.95   # CANL TVS junction x on wire

# Junction dots on horizontal CAN wires
dot(T1X, W_CH, VEH)   # CANH junction
dot(T2X, W_CL, VEH)   # CANL junction

# Drop wires from junctions down to TVS boxes
wire([(T1X, W_CH), (T1X, 10.55)], VEH, lw=1.8)   # CANH drop
wire([(T2X, W_CL), (T2X,  9.45)], VEH, lw=1.8)   # CANL drop

# TVS component boxes
# CANH TVS: box top = 10.55, height 0.60
rbox(T1X-0.55, 9.95, 1.2, 0.60, VEH, '#081408', lw=1.5)
t(T1X+0.05, 10.38, 'P6KE6.8CA', VEH, 6.5, bold=True)
t(T1X+0.05, 10.09, 'TVS  CANH', TXT2, 6, italic=True)

# CANL TVS: box top = 9.45, height 0.60
rbox(T2X-0.55, 8.85, 1.2, 0.60, VEH, '#081408', lw=1.5)
t(T2X+0.05, 9.28,  'P6KE6.8CA', VEH, 6.5, bold=True)
t(T2X+0.05, 8.99,  'TVS  CANL', TXT2, 6, italic=True)

# GND symbols below each TVS
gnd_sym(T1X, 9.63, GND)
gnd_sym(T2X, 8.53, GND)

# ─── SN65HVD230 Module (blue breakout, 3.3V, 6-pin) ──────────────────────────
...
# Left side pins (CAN bus side): CANH, CANL, 3V3, GND
...
# Right side pins (MCU side): CTX, CRX
...
# ─── 1N5819 Schottky (reverse polarity protection) ────────────────────────────
DX,DY=5.4,5.57
...
for i,(c,lbl) in enumerate([
        (VEH,'Vehicle CAN  (CANH / CANL  and  CTX / CRX to ESP32)'),
        (BLE,'Bluetooth LE  (NUS — wirelessly to Android)'),
        (P12,'+12V  from OBD-II coupler  (Pin 16)  →  Fuse  →  Schottky'),
        (P5, '+5V  regulated  (LM2596 out  →  ESP32 VIN)'),
        (P33,'+3.3V  (ESP32 3V3 pin  →  SN65HVD230 3V3)'),
...
for i,(c,note) in enumerate([
        ...
        (TXT2,'Chassis bus: future satellite ESP32 under seat (separate)')]):
